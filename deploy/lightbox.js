(function () {
  ...
  style.textContent =
    ...
    '.wp-lb{position:fixed;inset:0;z-index:2147483000;display:none;flex-direction:column;' +
      ...
      'background:rgba(20,20,20,0.94);opacity:0;transition:opacity .22s ease;cursor:zoom-out;}' +
    '.wp-lb.wp-open{opacity:1;}' +
    '.wp-lb img{max-width:92vw;max-height:82vh;width:auto;height:auto;display:block;' +
      'border:1px solid rgba(255,255,255,0.14);background:#FFFFFF;box-shadow:0 30px 90px rgba(0,0,0,0.55);' +
    ...
    '.wp-lb figcaption,.wp-lb .wp-cap{margin-top:18px;max-width:80vw;text-align:center;' +
      "font-family:'inclusive-sans',sans-serif;font-size:12px;letter-spacing:0.08em;" +
      'text-transform:uppercase;color:#F5F5F5;}' +
    '.wp-lb-close{position:fixed;top:18px;right:26px;font:300 34px/1 Arial,sans-serif;' +
      'color:#F5F5F5;cursor:pointer;user-select:none;opacity:0.85;}' +
